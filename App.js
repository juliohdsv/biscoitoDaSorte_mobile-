...
export default function App() {
  ...
  function toBreak(){    
    if(message === ""){
      let number = Math.floor(Math.random() * arrText.length);

      setImg(require("./assets/images/biscoitoAberto.png"));
      setMessage(`"${arrText[number]}"`); 
      return;
    }
  }
  ...
  return (
    <View className="flex-1 items-center justify-center">
      <Image 
        ...
        className="w-64 h-64"
      />
      <Text className="text-orange-500 text-lg font-semibold italic items-center m-16">
        {message}
      </Text>
      <TouchableOpacity 
        onPress={ ()=> toBreak()}
        className="w-52 h-10 items-center justify-center rounded-2xl border-2 border-orange-500"
      >
        <View className="w-full h-full items-center justify-center">
          <Text className="text-orange-600 font-semibold text-base">Quebrar Biscoito</Text>
        </View>
      </TouchableOpacity>
      <TouchableOpacity 
        onPress={ ()=> reset()}
        className="w-52 h-10 items-center justify-center rounded-2xl border-2 border-black-500 my-4"
      >
        <View className="w-full h-full items-center justify-center">
          <Text className="text-black font-semibold text-base">+ Biscoito</Text>
        </View>
      </TouchableOpacity>
    </View>
  );
